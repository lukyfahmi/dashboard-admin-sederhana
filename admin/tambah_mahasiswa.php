		...
		<form class="form-horizontal" method="post" enctype="multipart/form-data" action="konfirmasi_tambah_mahasiswa.php">
			<div class="card-body">
			   ...
			   <div class="form-group row">
					...
					<div class="col-sm-7">
						<input type="text" class="form-control" name="nim" id="nim" value="<?php if(!empty($_SESSION['nim'])){ echo $_SESSION['nim'];} ?>">
					</div>
				</div>
				<div class="form-group row">
					<label for="nama" class="col-sm-3 col-form-label">Nama</label>
					<div class="col-sm-7">
						<input type="text" class="form-control" name="nama" id="nama" value="<?php if(!empty($_SESSION['nama'])){ echo $_SESSION['nama'];} ?>">
					</div>
				</div>
				<div class="form-group row">
					...
					<div class="col-sm-7">
						<select class="form-control" id="jurusan" name="jurusan">
							<option value="0">- Pilih Jurusan -</option>
							   <?php
							   $sql_j = "select `kode_jurusan`, `jurusan` from `jurusan` order by `kode_jurusan`";
							   $query_j = mysqli_query($koneksi,$sql_j);
									while($data_j = mysqli_fetch_row($query_j)){
										$kode_jurusan = $data_j[0];
										$jurusan = $data_j[1];
							   ?>
							<option value="<?php echo $kode_jurusan;?>"<?php if(!empty($_SESSION['jurusan'])){if($kode_jurusan==$_SESSION['jurusan']){?> selected="selected" <?php }}?>>
								<?php echo $jurusan;?>
								<?php }?>
							</option>
						</select>
				...
				<div class="form-group row">
					<label for="hobi" class="col-sm-3 col-form-label">Hobi</label>
						<div class="col-sm-7">
						   <?php 
						   $sql_h = "select `kode_hobi`,`hobi` from `hobi` order by `kode_hobi`";
